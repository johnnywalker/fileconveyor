...
#     PERSISTENT_DATA_DB = None
#     SYNCED_FILES_DB = None
#     FSMONITOR_DB = None
#     WORKING_DIR = None
#     MAX_FILES_IN_PIPELINE = None
#     MAX_SIMULTANEOUS_PROCESSORCHAINS = None
#     MAX_SIMULTANEOUS_TRANSPORTERS = None
#     MAX_TRANSPORTER_QUEUE_SIZE = None
#     QUEUE_PROCESS_BATCH_SIZE = None
#     CALLBACKS_CONSOLE_OUTPUT = None
...
from fileconveyor.transport_notifier import get_notifier
...
class Arbitrator(object):
    ...
    def __init__(self, config_xml):
        ...
        self.config_errors = self.config.load(config_xml)
        self.logger.warning("Loaded config.")
        if self.config_errors > 0:
            self.logger.error("Cannot continue, please fix the errors in the config xml first.")
        ...
        for server in self.config.servers.keys():
            ...
            modulename = "fileconveyor.transporters.transporter_" + transporter_name
            try:
    ...
    def __setup(self):
        self.processor_chain_factory = ProcessorChainFactory("Arbitrator", settings.WORKING_DIR)
        ...
        self.pipeline_queue = PersistentQueue("pipeline_queue", settings.PERSISTENT_DATA_DB)
        self.logger.warning("Setup: initialized 'pipeline' persistent queue, contains %d items." % (self.pipeline_queue.qsize()))
        self.files_in_pipeline =  PersistentList("pipeline_list", settings.PERSISTENT_DATA_DB)
        ...
        self.failed_files = PersistentList("failed_files_list", settings.PERSISTENT_DATA_DB)
        ...
        self.dbcon = sqlite3.connect(settings.SYNCED_FILES_DB)
        ...
        self.fsmonitor = fsmonitor_class(self.fsmonitor_callback, True, True, self.config.ignored_dirs.split(":"), settings.FSMONITOR_DB, "Arbitrator")
    ...
    def run(self):
        ...
        self.notifier.stop()
        self.notifier.join()
        self.logger.warning('Stopped Transport Notifier.')
    ...
    def __process_pipeline_queue(self):
        ...
        while processed < settings.QUEUE_PROCESS_BATCH_SIZE and self.pipeline_queue.qsize() > 0 and len(self.files_in_pipeline) < settings.MAX_FILES_IN_PIPELINE:
            self.lock.acquire()
    ...
    def __process_filter_queue(self):
        ...
        while processed < settings.QUEUE_PROCESS_BATCH_SIZE and self.filter_queue.qsize() > 0:
    ...
    def __process_process_queue(self):
        ...
        while processed< settings.QUEUE_PROCESS_BATCH_SIZE and self.process_queue.qsize() > 0 and self.processorchains_running < settings.MAX_SIMULTANEOUS_PROCESSORCHAINS:
    ...
    def __process_transport_queues(self):
        for server in self.config.servers.keys():
            ...
            while processed < settings.QUEUE_PROCESS_BATCH_SIZE and self.transport_queue[server].qsize() > 0:
                ...
                if not transporter is None:
                    ...
                    src = output_file
                    relative_paths = [settings.WORKING_DIR, self.config.sources[rule["source"]]["scan_path"]]
    ...
    def __process_db_queue(self):
        ...
        while processed < settings.QUEUE_PROCESS_BATCH_SIZE and self.db_queue.qsize() > 0:
            # DB queue -> database.
            self.lock.acquire()
    ...
    def __process_retry_queue(self):
        ...
        while processed < settings.QUEUE_PROCESS_BATCH_SIZE and self.retry_queue.qsize() > 0:
    ...
    def __allow_retry(self):
        num_failed_files = len(self.failed_files)
        should_retry = self.last_retry + settings.RETRY_INTERVAL < time.time()
        pipeline_queue_almost_empty = self.pipeline_queue < settings.MAX_FILES_IN_PIPELINE
        
        if num_failed_files > 0 and (should_retry or pipeline_queue_almost_empty):
            failed_items = []

            processed = 0
            while processed < settings.QUEUE_PROCESS_BATCH_SIZE and processed < len(self.failed_files):
    ...
    def __get_transporter(self, server):
        ...
        for id in range(0, len(self.transporters[server])):
            ...
            # transporter's queue.
            if transporter.qsize() <= settings.MAX_TRANSPORTER_QUEUE_SIZE:
        ...
        if not self.transporters_running < settings.MAX_SIMULTANEOUS_TRANSPORTERS:
    ...
    def __create_transporter(self, server):
        ...
        transporter_modulename = "fileconveyor.transporters.transporter_" + transporter_name
        ...
        try:
            transporter = transporter_class(settings, self.transporter_callback, self.transporter_error_callback, self.notifier, "Arbitrator")
    ...
    def fsmonitor_callback(self, monitored_path, event_path, event, discovered_through):
        ...
        if settings.CALLBACKS_CONSOLE_OUTPUT:
    ...
    def handle_signal(self, signalNumber, frame):
        if signalNumber in (signal.SIGTERM, signal.SIGINT, signal.SIGTSTP):
            self.stop()

    def clean_up_working_dir(self):
        for root, dirs, files in os.walk(settings.WORKING_DIR, topdown=False):
            for name in files:
                os.remove(os.path.join(root, name))
            for name in dirs:
                os.rmdir(os.path.join(root, name))
        self.logger.info("Cleaned up the working directory '%s'." % (settings.WORKING_DIR))


def run_file_conveyor():
    # Read in the settings and XML configuration on STDIN
    # These 2 records will be separated by a null byte
    data = sys.stdin.read()
    if len(data) == 0:
        exit(1)
    (settings_code, config_xml) = data.split('\0')
    settings_code = 'global settings\n' + settings_code
    exec settings_code

    try:
        arbitrator = Arbitrator(config_xml)
